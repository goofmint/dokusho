...
final class EpubReaderViewModel {
    ...
    func load() async {
        ...
        let initialLocation = await resolveInitialLocation(in: publication)
    ...
    /// resume 位置を決定する。Komga の readProgress.page を全体進捗へ近似変換し、
    /// publication の positions のうち最も近いものを初期 locator とする。
    private func resolveInitialLocation(in publication: Publication) async -> Locator? {
        guard let progress = book.readProgress, !progress.completed else {
            return nil
        }
        // (page - 1) / max(pagesCount - 1, 1) で 0.0〜1.0 の全体進捗に近似。
        let denominator = Double(max(pagesCount - 1, 1))
        let target = Double(max(progress.page - 1, 0)) / denominator
        ...
        case let .success(positions) where !positions.isEmpty:
            let best = positions.min { lhs, rhs in
            ...
            return best
        default:
            // positions を取得できない場合はサイレントに先頭へは倒さず、
            // 近似 locator を作れないため nil（先頭表示）とする。
            return nil
        }
    }
